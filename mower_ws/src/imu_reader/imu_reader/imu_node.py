...
from imu_reader.drivers.imu_factory import IMUFactory, ImuDriver
...
class IMUNode(Node):
    ...
    Publishes:
        /imu/data (sensor_msgs/Imu): IMU data at configured rate
        /diagnostics (DiagnosticArray): Health status at 1Hz
    
    Parameters:
        imu_type (str): IMU type - "mpu6050" or "bno085" (default: "bno085")
        i2c_bus (int): I2C bus number (default: 1)
        i2c_address (int): I2C address in decimal (default: 74 = 0x4A for BNO085)
        publish_rate (float): Publishing rate in Hz (default: 20.0)
    ...
    def __init__(self):
        ...
        # Declare parameters
        self.declare_parameter('imu_type', 'bno085')
        self.declare_parameter('i2c_bus', 1)
        self.declare_parameter('i2c_address', 74)  # 0x4A for BNO085, 104 (0x68) for MPU6050
        ...
        # Handle i2c_address - may come as string from launch config
        addr_raw = self.get_parameter('i2c_address').value
        if isinstance(addr_raw, str):
            self._address = int(addr_raw)
        else:
            self._address = int(addr_raw)
        ...
        # QoS for sensor data - use RELIABLE so ros2 topic echo works without flags
        ...
        self._diag_publish_count = 0  # Separate counter for diagnostics rate reporting
        self._error_count = 0
    ...
    def _publish_callback(self) -> None:
        ...
        try:
            ...
            # Log first few publishes and then periodically
            if self._publish_count <= 3 or self._publish_count % 500 == 0:
                self.get_logger().info(
            ...
            self.get_logger().warn(f"IMU read/publish error: {e}")
            self._error_count += 1
    
    def _diagnostics_callback(self) -> None:
        """Timer callback to publish diagnostics at 1Hz."""
        msg = DiagnosticArray()
        msg.header.stamp = self.get_clock().now().to_msg()
        ...
        if self._driver is None:
            ...
            if self._imu_type.lower() == 'bno085':
                ...
                if driver_status.get('is_calibrated', False):
                    ...
                    status.message = "Running (calibration in progress)"
            else:
            ...
            # Calculate actual publish rate (messages in last 1 second)
            actual_rate = self._diag_publish_count
            self._diag_publish_count = 0  # Reset only the diag counter
            
            # Add key-values from driver status
            status.values = [
                KeyValue(key="imu_type", value=str(driver_status.get('imu_type', self._imu_type))),
                ...
                KeyValue(key="total_published", value=str(self._publish_count)),
                KeyValue(key="error_count", value=str(self._error_count)),
            ]
...
def main(args=None):
    ...
    try:
        ...
        try:
            ...
            pass  # Already shut down from signal handler
